...
class GameMainWindow(QMainWindow):
    ...
    def prepare_window(self):
        ...
        self.ui.doit.clicked.connect(self.gen_map)
        self.ui.map_size.valueChanged.connect(self.resize_map)
    ...
    def resize_map(self, value):
        ...
        self.ui.map_view.fitInView(QRectF(0.0, 0.0, self.map_fit, self.map_fit))
        if self.myself:
            self.ui.map_view.centerOn(self.myself)
    ...
    def pause(self):
        if self.paused:
            self.paused = False
            self.ui.doit.setEnabled(False)
            self.ui.cheat_map.setEnabled(False)
            self.ui.cheat_monsters.setEnabled(False)
        else:
            self.paused = True
            self.ui.doit.setEnabled(True)
            self.ui.cheat_map.setEnabled(True)
            self.ui.cheat_monsters.setEnabled(True)

    def toggle_cheat_map(self):
        do_cheat = self.ui.cheat_map.isChecked()
        if len(self.data):
    ...
    def gen_map(self, start_over=True):
        ...
        if self.ui.cheat_map.isChecked():
            print('running map cheat')
    ...
    def redraw_self(self, start=None, end=None):
        ...
        self.ui.map_view.fitInView(QRectF(0, 0, self.map_fit, self.map_fit))
        self.ui.map_view.centerOn(self.myself)
        ...
        if self.monsters:
            for m in self.monsters:
                ...
                if self.ui.cheat_monsters.isChecked() or [row, column] in self.current_visible:
                    m.visible = True
                    m.update()
        ...
        for r in range(0, 100):
            for c in range(0, 100):
                ...
                if cell.space_type in ['room', 'exit']:
                    ...
                    self.hallways[str(cell.location)] = 'hallway'
    ...
    def set_known(self):
        self.ui.rooms_discovered.setText("%s/%s" % (len(self.rooms_known), len(self.rooms)))
        self.ui.hallway_discovered.setText("%s/%s" % (len(self.halls_known), len(self.hallways)))
        self.ui.total_kills.setText("%s/%s" % (self.kills, len(self.monsters)))
        self.ui.cleared_count.setText(str(self.exits_found))
        point_total = self.total_points()
        self.ui.cur_points.setText(str(point_total))
        self.extra_lives = int(point_total/1000) - self.spent_lives
        self.ui.cur_lives.setText(str(self.extra_lives))
        if self.extra_lives:
            self.ui.go_again.setEnabled(True)
        else:
            self.ui.go_again.setEnabled(False)
